...
class YiiDebugToolbarRoute extends CLogRoute
{
    ...
    public $rFilters=array('site/captcha');//过滤get中r参数
    public $getFilters=array();//过滤get中的参数
    ...
    public function init()
    {
        ...
        {
            foreach ($this->getFilters as $k=>$value) {
                if(isset($_GET[$k])&&$_GET[$k]==$value)
                {
                    $goodGet = false;
                    break;
                }
        ...
        $this->enabled && $this->enabled = ($this->allowIp(Yii::app()->request->userHostAddress)
                && !Yii::app()->getRequest()->getIsAjaxRequest() && (Yii::app() instanceof CWebApplication)&&!in_array($_GET['r'],$this->rFilters)&&$goodGet);
