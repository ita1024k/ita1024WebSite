...
class UserController extends Controller
{
	...
	public function actionCreate()
	{
			...
			if(is_object($uploadedimage) && get_class($uploadedimage)==='CUploadedFile')
			{
				...
				$model->logo='./uploads/img_app/' . $filename . '.' . $ext;
                        ...
                        if($model->save())
                        {
                                ...
                                $user_id = yii::app()->db->createCommand("SELECT user_id FROM act_user WHERE media_id = '$media_id'")->queryScalar();
                                $sql_insert = "INSERT INTO AuthAssignment (`itemname`,`userid`,`data`) VALUES ('Operator','$user_id','N;')";
        ...
        public function actionCheckThrough(){
            ...
            if($user_id){
                    $sql_update = "UPDATE act_user SET status=$status WHERE user_id = $user_id";
